...
namespace MPT.CSI.API.Core.Program.ModelBehavior.Definition.LoadCase
{
    ...
    public class StaticNonlinearStaged : CSiApiBase, IStaticNonlinearStaged
    {
            ...
            ref double[] scaleFactors)
        {
            ...
            _callCode = _sapModel.LoadCases.StaticNonlinearStaged.GetStageData_2(name, ref stage, ref numberOfOperations, 
            ...
            objectTypes = new eStagedConstructionObject[numberOfOperations - 1];
            for (int i = 0; i < numberOfOperations; i++)
            {
                objectTypes[i] = EnumLibrary.ConvertStringToEnumByDescription<eStagedConstructionObject>(csiObjectTypes[i]);
            }
            ...
            double[] scaleFactors)
        {
            ...
            string[] csiObjectTypes = new string[numberOfOperations - 1];
            for (int i = 0; i < numberOfOperations; i++)
            {
                csiObjectTypes[i] = objectTypes[i].ToString();
            }

            _callCode = _sapModel.LoadCases.StaticNonlinearStaged.SetStageData_2(name, stage, numberOfOperations,
                ref csiOperations, ref csiObjectTypes, ref nameObjects, ref ages, ref loadOrObjectTypes, ref loadOrObjectNames, ref scaleFactors);
            if (throwCurrentApiException(_callCode)) { throw new CSiException(); }
            // TODO: Handle loadOrObjectTypes & loadOrObjectNames in OOP wrapper as these are dependent upon objectTypes
        }
        ...
        public void GetStageDefinitions(string name,
            ...
            ref string[] comment)
        {
            _callCode = _sapModel.LoadCases.StaticNonlinearStaged.GetStageDefinitions_2(name, ref numberOfStages, ref duration, ref outputIsToBeSaved, ref nameOutput, ref comment);
            if (throwCurrentApiException(_callCode)) { throw new CSiException(); }
        }
        ...
        public void SetStageDefinitions(string name,
            ...
            string[] comment)
        {
            _callCode = _sapModel.LoadCases.StaticNonlinearStaged.SetStageDefinitions_2(name, numberOfStages, ref duration, ref outputIsToBeSaved, ref nameOutput, ref comment);
            if (throwCurrentApiException(_callCode)) { throw new CSiException(); }
        }
